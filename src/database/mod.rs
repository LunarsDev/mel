    ...
    unsafe {
        match &CONNECTION {
            ...
            None => {
                ...
                //     "{}/{}",
                //     MelConfig::get_database_connection_string(),
                //     MelConfig::get_database_name()
                // );

                let opts = mysql::OptsBuilder::new()
                    .ip_or_hostname(Some(format!("{}", MelConfig::get_database_host())))
                    .pass(Some(format!("{}", MelConfig::get_database_pass())))
                    .user(Some(format!("{}", MelConfig::get_database_user())))
                    .db_name(Some(format!("{}", MelConfig::get_database_name())));
        ...
        Err(err) => {
            if let Error::MySqlError(err) = err {
                if err.code == 1049 {
                    ...
                    conn.query_drop(r"CREATE DATABASE IF NOT EXISTS mel")?;
